...
export default function CompassApp() {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 text-slate-100 overflow-hidden">
      ...
      {/* Header — title and instructions, top-left */}
      <header className="absolute top-0 left-0 z-10 p-6 pointer-events-none">
        <h1 className="text-2xl font-bold tracking-tight">
          Simulacras Novas
        </h1>
        <p className="text-[11px] text-pink-300/80 mt-0.5 font-mono tracking-wide">
      ...
      {/* Axis picker + presets + CTA, top-right */}
      <div className="absolute top-6 right-6 z-10 flex flex-col gap-3 pointer-events-none w-[320px]">
        <AxisPicker x={x} y={y} z={z} onChange={setAxis} />

        <div className="bg-slate-900/85 backdrop-blur border border-slate-700 rounded-lg p-3 shadow-xl pointer-events-auto">
        ...
        {/* CTA — Take the assessment, sized to draw the eye */}
      ...
      <footer className="absolute bottom-4 left-6 z-10 text-[10px] text-slate-600 pointer-events-none font-mono">
        v0.12 — Simulacras Novas · 55 thinkers · 8 axes · selectable
